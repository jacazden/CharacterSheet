...
int Skill::save(boost::filesystem::path dir,bool append)
{
        ...
        if((exists(dir) & is_directory(dir)) || (exists(dir)&!is_directory(dir)&append))
        {
                ...
                ofs<<"\t<attribute>"<<attr->Name()<<"</attribute>"<<std::endl;
                ofs<<"</skill>"<<std::endl;
                ofs.close();
                return 0;
        }
...
Skill* Skill::load(boost::filesystem::path filepath,int seek)
{
        using boost::filesystem::exists;
        using boost::filesystem::is_directory;
        using std::regex;
                        ...
                        while(ifs>>std::noskipws>>ch)
                        {
                                if(ch=="<")
                                {
                                        if(instring!="")
                                        {
                                                ...
                                                else if(regex_match(block.last()->tag,rgx,A))
                                                {
                                                        attr = new Attribute(instring,"",0,false,
