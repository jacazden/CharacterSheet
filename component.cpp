...
std::string Component::Description() { return description; }
std::string Component::Description(std::string input) { description=input; return description; }

int Component::Value() { return value; }
int Component::Value(int input) { value=input; return value; }

int Component::save(boost::filesystem::path dir,bool append)
{
        ...
        if((exists(dir) & is_directory(dir)) || (exists(dir)&!is_directory(dir)&append))
        {
                ...
                ofs<<"</component>"<<std::endl;
                ofs.close();
                return 0;
        }
        return -1;
}
Component* Component::load(boost::filesystem::path filepath,int seek)
{
        using boost::filesystem::exists;
        using boost::filesystem::is_directory;
        using std::regex;
        ...
        if(exists(filepath))
        {
                if(rcmp(filepath.extension().string(),".cmp"))
                {
